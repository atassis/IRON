...
def my_matmul(
    ...
    if m_stationary:
        ...
        assert (
            not use_larger_internal_buffer
        ), "m_stationary: prio_accuracy not supported"
        M_band = M // n_aie_cols
        assert (
            M % (m * n_aie_rows * n_aie_cols) == 0
        ...
        dev_ty = (
            NPU2()
        )  # m_stationary is npu2-only (matches route_b m_stationary_iron.py)
        gemm_object_ms = (
        ...
        zero_kernel = Kernel(
            f"{func_prefix}zero_{dtype_out_str}", gemm_object_ms, [C_l1_ty]
        )
        ...
        for col in range(n_aie_cols):
            ...
            A_l3l2_fifos[col] = ObjectFifo(
                A_l2_ty_ms, name=f"A_L3L2_{col}", depth=ms_depth
            )
            ...
            B_l3l2_fifos[col] = ObjectFifo(
                B_l2_ty_ms, name=f"B_L3L2_{col}", depth=ms_depth
            )
            if b_col_maj:
        ...
        def ms_sequence(A, B, C, A_prods, B_prods, C_conses):
            for c in range(n_chunks):
                tg = TaskGroup()
                for col in range(n_aie_cols):
                    A_prods[col].fill(A, A_tiles[col], group=tg)
                    B_prods[col].fill(B, B_tiles[c], group=tg)
                for col in range(n_aie_cols):
                    C_conses[col].drain(
                        C, C_tiles[col * n_chunks + c], group=tg, wait=True
                    )
                tg.finish()
        ...
        ms_program = Program(dev_ty, rt, workers=ms_workers)
        maybe_enable_trace(ms_program, trace_size, ms_workers)
        return ms_program.resolve_program()
