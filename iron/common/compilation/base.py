...
def _tool_runs(path):
    ...
    is present but cannot run -- e.g. the pinned peano `llvm-nm`, whose shared
    libLLVM is missing `llvm::opt::OptTable` typeinfo, so it exits 127 with an
    `undefined symbol ... LLVM_21.0` loader error and emits nothing."""
    try:
        return (
...
def _find_working_tool(name, peano_dir, mlir_aie_dir):
    ...
    _find_tool returns the FIRST *existing* binary even if it cannot run; the
    pinned peano llvm-nm is exactly such a binary. Used silently in a
    `nm | awk > map` pipeline it yields an EMPTY symbol map (the pipe's exit
    status is awk's, so nm's failure is masked) -> the fusion symbol prefix is
    never applied -> `undefined symbol: op<idx>_<sym>` at the per-core link."""
    candidates = [
        Path(peano_dir) / "bin" / name,
        Path(mlir_aie_dir) / "bin" / name,
    ]
