...
def element_size_bytes(ty: ir.Type) -> int:
    ...
    every buffer is bf16, so any other buffer had its element count silently divided by 2 -- an
    int4 weight buffer of 1671168 bytes was reported as 835584 elements. The arena is
    ...
    type it actually occupies (i8), which is what the quantized GEMV does: its int4 row is not N
    elements of one type but ceil(K/2) packed nibbles plus K/group f32 scales, so no element
    count describes it and only the byte count is meaningful.
    """
